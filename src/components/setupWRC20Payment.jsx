          ...
          return (
            <Grid container justify="center" alignItems="center" direction="row">
              ...
              <Grid item xs={4} style={{borderLeft: '1px solid #dedede'}} align='center'>
                <div></div>
                <Typography variant='title' noWrap>
                  {this.props.sendWRC20Symbol ? selectedAddress.wrc20Tokens.filter((token) => { return token.symbol == this.props.sendWRC20Symbol; })[0].balance+" "+this.props.sendWRC20Symbol : ''}
                </Typography>
                <Typography variant='subheading' noWrap>
                  {this.props.sendWRC20Symbol ? 'Available Balance' : 'Select a token'}
                </Typography>
              </Grid>
            </Grid>)
        }} >
        {this.props.wanAddresses.map((address) => {
          return (
            <MenuItem value={address.publicAddress} key={address.publicAddress}>
              ...
              <ListItemSecondaryAction style={{right: '24px'}}>
                { this.props.sendWRC20Symbol ? address.wrc20Tokens.filter((token) => { return token.symbol == this.props.sendWRC20Symbol; })[0].balance+" "+this.props.sendWRC20Symbol : ''}
              </ListItemSecondaryAction>
            </MenuItem>
          )
        })}
          ...
          <Grid item xs={12} align='left' style={{marginTop: '24px'}}>
            ...
            <TextField required fullWidth={false} color="textSecondary" error={this.props.amountError} style={{minWidth: '300px', maxWidth: '400px', marginTop: '0px'}}
              ...
              helperText={this.props.amountErrorMessage} onBlur={(event) => { this.props.validateField(event, 'amount'); }} InputProps={{
                endAdornment: <InputAdornment position="end">{this.props.sendWRC20Symbol}</InputAdornment>,
              }} />
          </Grid>
          <Grid item xs={12} align='left' style={{marginTop: '24px'}}>
            <Typography variant="subheading">
