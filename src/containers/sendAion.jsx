...
let SendAion = createReactClass({
  getInitialState() {
    return {
      ...
      disclaimer:
        "CryptoCurve accepts no responsibility for funds transferred to an incorrect public address. Please ensure that the address entered is the correct Aion Public Address of your recipient."
    };
  },
    ...
    return (
      <Grid container justify="center" alignItems="center" direction="row">
        <Grid
          ...
          align="left"
        >
          <PageTItle theme={this.props.theme} root={'Accounts > Aion'} screen={'Send'} />
        </Grid>
  ...
  selectContact(event) {
    ...
    this.setState({
      contactValue: selectedContact.primaryAionAddress,
  ...
  proceedClicked() {
    ...
    if (this.validateSetupPayment()) {
      ...
      completed[0] = true;
      this.setState({
        currentScreen: "confirmAionPayment",
        activeStep: 1,
        completed
      });
    }
  ...
  sendAionReturned(error, data) {
    ...
    if (data.success) {
      this.setState({
        currentScreen: "completeAionPayment",
        activeStep: 2,
        completed,
  ...
  validateGas(value) {
    ...
    if (value === "") {
      this.setState({ gweiError: true, gweiErrorMessage: "Gas is required" });
      return false;
    } else if (!this.isNumeric(value)) {
