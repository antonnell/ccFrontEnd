...
let SendWanchain = createReactClass({
  getInitialState() {
    return {
      ...
      disclaimer:
        "CryptoCurve accepts no responsibility for funds transferred to an incorrect public address. Please ensure that the address entered is the correct Wanchain Public Address of your recipient."
    };
  },
  ...
  proceedClicked() {
    ...
    if (this.validateSetupPayment()) {
      ...
      completed[0] = true;
      this.setState({
        currentScreen: "confirmWanchainPayment",
        activeStep: 1,
        completed
      });
    }
  ...
  sendWanchainReturned(error, data) {
    ...
    if (data.success) {
      this.setState({
        currentScreen: "completeWanchainPayment",
        activeStep: 2,
        completed,
